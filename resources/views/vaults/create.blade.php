...
<div class="container mx-auto py-6">
    ...
    <form action="{{ route('vaults.store') }}" method="POST">
        ...
        <div class="mb-4">
            ...
            <input type="text" name="vault_name" class="border rounded w-full px-2 py-1" value="{{ old('vault_name') }}">
        </div>

        <div class="mb-4">
            <label class="block mb-1">Vault Type</label>
            <select name="vault_type" class="border rounded w-full px-2 py-1">
                <option value="MAIN" {{ old('vault_type') == 'MAIN' ? 'selected' : '' }}>MAIN</option>
                <option value="SUB" {{ old('vault_type') == 'SUB' ? 'selected' : '' }}>SUB</option>
            </select>
        </div>

        <div class="mb-4">
            <label class="block mb-1">Current Balance</label>
            <input type="number" name="current_balance_cents" class="border rounded w-full px-2 py-1" value="{{ old('current_balance_cents', 0) }}">
        </div>
