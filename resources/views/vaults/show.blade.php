    ...
    <div class="max-w-3xl mx-auto bg-white shadow-lg rounded-lg p-8">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            ...
            <div>
                ...
                <p class="text-lg font-semibold text-gray-900">{{ $vault->vault_name }}</p>
            </div>

            <!-- Vault Type -->
            <div>
                <p class="text-sm font-medium text-gray-600">ປະເພດຄັງ</p>
                <p class="text-lg font-semibold text-gray-900">{{ $vault->vault_type }}</p>
            </div>

            <!-- Current Balance -->
            <div>
                <p class="text-sm font-medium text-gray-600">ຍອດເງິນ</p>
                <p class="text-lg font-semibold text-gray-900">{{ number_format($vault->current_balance, 2) }}</p>
            </div>
            ...
            <div>
                ...
                <p class="text-gray-700">{{ $vault->created_at->format('d-m-Y H:i') }}</p>
            </div>

            <!-- Updated At -->
            <div>
                <p class="text-sm font-medium text-gray-600">ແກ້ໄຂເມື່ອ</p>
                <p class="text-gray-700">{{ $vault->updated_at->format('d-m-Y H:i') }}</p>
            </div>
        </div>

        <!-- Action Buttons -->
        <div class="flex justify-end space-x-3">
            <a href="{{ route('vaults.index') }}" class="btn-secondary">ກັບຄືນ</a>
            <a href="{{ route('vaults.edit', $vault) }}" class="btn-primary">ແກ້ໄຂຄັງ</a>
        </div>
    </div>
</div>
@endsection
